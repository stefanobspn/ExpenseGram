...
from src.core.parser import parse_transaction
...
async def add_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Usage: `/addcategory <category_name>`", parse_mode="Markdown")
        return
    ...
    if success:
        await update.message.reply_text(f"✅ Category `{cat_name.lower()}` added.", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"⚠️ Category `{cat_name.lower()}` already exists or is invalid.", parse_mode="Markdown")
...
async def del_category(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Usage: `/delcategory <category_name>`", parse_mode="Markdown")
        return
...
async def list_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """List all accounts alphabetically."""
    db = get_db(context)
...
async def add_account(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Usage: `/addaccount <account_name>`", parse_mode="Markdown")
        return
    ...
    if success:
        await update.message.reply_text(f"✅ Account `{acc_name.lower()}` added.", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"⚠️ Account `{acc_name.lower()}` already exists or is invalid.", parse_mode="Markdown")
...
async def del_account(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Usage: `/delaccount <account_name>`", parse_mode="Markdown")
        return
...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args:
        ...
        if len(arg) == 7 and arg[4] == '-':
            try:
                ...
                await update.message.reply_text("❌ Invalid format. Use YYYY-MM (e.g. 2026-05) or MM (e.g. 05).")
                return
        else:
            # Parse MM
            try:
                ...
                await update.message.reply_text("❌ Invalid month. Provide 1-12 (e.g. 05) or YYYY-MM.")
                return
    ...
    if not rows:
        await update.message.reply_text(f"📅 No transactions found for **{month_name} {year}**.", parse_mode="Markdown")
        return
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args:
        try:
            ...
            await update.message.reply_text("❌ Invalid limit. Provide a positive number.")
            return
    ...
    for t in txs:
        if isinstance(t["created_at"], str):
            try:
                ...
                date_str = datetime.strptime(dt_str, "%Y-%m-%d %H:%M:%S").strftime("%Y-%m-%d %H:%M")
            except Exception:
...
async def delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text("Usage: `/delete <transaction_id>`", parse_mode="Markdown")
        return

    try:
        tx_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("❌ Invalid transaction ID. Must be an integer.")
        return

    db = get_db(context)
    success = db.delete_transaction(tx_id)
    if success:
        await update.message.reply_text(f"✅ Transaction `ID: {tx_id}` deleted successfully.", parse_mode="Markdown")
    else:
        await update.message.reply_text(f"❌ Transaction `ID: {tx_id}` not found.", parse_mode="Markdown")


@owner_required
async def undo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Undo the last logged transaction."""
    db = get_db(context)
    tx = db.delete_last_transaction()
    if not tx:
        await update.message.reply_text("⏮️ No transactions found to undo.", parse_mode="Markdown")
        return
...
async def export_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    writer.writerow(["ID", "Amount", "Type", "Account", "Category", "Description", "Created At"])
    for t in txs:
        writer.writerow([t["id"], t["amount"], t["type"], t["account"], t["category"], t["description"] or "", t["created_at"]])
